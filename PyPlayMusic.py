...
if len(args) > 1:
    ...
    try:
        ...
        print('If it isn\'t missing then some dependencies need to be installed.')
        sys.exit(1)
else:
    try:
...
class MainWindow(shared.Centerable, Tkinter.Tk):
    ...
    def __init__(self):
        ...
        # Add device ID if one exists
        devices = mobile_client.get_registered_devices()
        if devices:
            device = devices[0]
        ...
        self.album_image.grid(column=0, row=1, rowspan=2)
        #self.fileinfo.grid(column=1, columnspan=1, row=1)
        self.fileinfo.grid(column=2, row=2, sticky='NW')
        self.controls_frame.grid(column=0, columnspan=3, row=3)
    ...
    def track_matches(self, track):
        ...
        return re.search(self.entry_variable.get().replace(' ', '|').replace('+', ' '), search_field, flags=re.I)
    ...
    def get_playlist_tracks(self, playlist_dict_tracks):
        """
        Gets TrackList from list of tracks in playlist dictionary.
